...
abstract class BaseArticleActivity<VB : ViewBinding>(inflater: InflateActivity<VB>) :
    BaseActivity<VB>(inflater) {
    lateinit var component: ArticleSubComponent

    override fun onBaseCreateSubComponent() {
        component =
            (applicationContext as BaseApp).applicationComponent.articleSubComponentFactory()
                .create()
